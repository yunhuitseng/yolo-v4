...
def transform(image, location=None):
    ...
    x1 = max(0, x)
    y1 = max(0, y)
    x2 = x+w if x+w < image.shape[1] else image.shape[1]
    y2 = y+h if y+h < image.shape[0] else image.shape[0]
    approx = np.float32([[x1, y1], [x2, y1], [x2, y2], [x1, y2]])
...
def parser():
    ...
    parser.add_argument("--input", type=str, default="tests/image16.jpg",
...
def image_detection(image_path, network, class_names, class_colors, thresh):
    ...
    for label, confidence, bbox in detections:
        bbox_adjusted = darknet.convert2original(image, bbox, height, width)
        bbox_list.append(bbox_adjusted)
        detections_adjusted.append((str(label), confidence, bbox_adjusted))
        image = darknet.draw_boxes(detections_adjusted, image, class_colors)
    return cv2.cvtColor(image, cv2.COLOR_BGR2RGB), bbox_list
...
def main(args):
    ...
    while True:
        ...
        prev_time = time.time()
        image, rect = image_detection(image_name, network, class_names, class_colors, args.thresh)
        from queue import Queue

        img_for_decode = Queue()
        for x, y, w, h in rect:
            dst = transform(image=src, location=(int(round(x - (w / 2))), int(round(y - (h / 2) - (h / 45) * 4)), w, h + (h / 45) * 8))
            img_for_decode.put(dst)
